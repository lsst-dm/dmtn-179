...
%
\begin{equation}
\hat{D}_{AL+d} = \frac{\hat{D}_Z}{F_D}\cdot\frac{\abs{\hat{P}_r}}{\hat{P}_r}
\end{equation}
%
\begin{align}
...
correctional step on the difference image. Let's consider the without
pre-convolution kernel case first ((\hat{P}_{pre}=1\) at all frequencies) 
%Note: looks like c_2 the
We expect high frequency noise in the science image convolution case
when \(\sigma_r \gg \sigma_n\) can happen. In this case the numerical
noise in the tails of \(\hat{P}_{mk}\) is amplified and \(\hat{K}\)
may not converge to its high frequency constant limit value (\(F_n/\sigma_n\)).
The practical problem with using the two-kernel ( decorrelation afterburner is exactly the same as the calculation of the ZOGY difference image 
% 
