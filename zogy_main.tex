...
space, the matching kernel is ideally the quotient of the two image
PSFs (\Cref{eq:Dal}).
%
\begin{equation}
  \hat{D}_{AL} =
...
\end{equation}
%
\par The decorrelation afterburner is a correction, applied in frequency
space onto the difference image (\Cref{eq:K}) to whiten the noise in
image space.
%
\begin{align}
\hat{K}  &= \frac
...
whitening theoretically leads to the same detection statistics.  In
the AL case the role of the two images are not symmetric, the template
PSF is matched to the science image, and the science image is left
intact. Assuming that the AL optimization finds the perfect matching
kernel, it should be \(\hat{P}_n/\hat{P}_r\). Indeed, considering the
score image, the difference between the AL and ZOGY images are only a
factor of \(\hat{P}_r/\abs{\hat{P}_r}\). Compared to the AL, in the
ZOGY case both the difference image and its PSF carry an extra
...
\begin{align}
 ...
 &= F_D \frac
  ...
  {\sqrt{\frac{\sigma_n^2}{F_n^2} +
      \frac{\sigma_r^2}{F_r^2}\frac{\abs{\hat{P}_n}^2}{\abs{\hat{P}_r}^2}}} = \\
  &= F_D \hat{D}_{AL+d} \cdot \frac{\overline{\hat{P}_n}
  \sqrt{\frac{\sigma_n^2}{F_n^2} + \frac{\sigma_r^2}{F_r^2}}}
  {\sqrt{\frac{\sigma_n^2}{F_n^2} +
      \frac{\sigma_r^2}{F_r^2}\abs{\frac{\hat{P}_n}{\hat{P}_r}}^2}} =
F_D \hat{D}_{AL+d}\overline{\hat{P}_{AL+d}}
\label{eq:DalDzPrPr}
\end{align}
%
\begin{equation}
\hat{D}_{AL+d} = \frac{\hat{D}_Z}{_D}\cdot\frac{\abs{\hat{P}_r}}{\hat{P}_r}
\end{equation}
%
\begin{align}
...
\end{align}
%
\section{The decorrelation afterburner}
The decorrelation afterburner was created as a post-processing
correctional step on the difference image. Let's consider it first
without the pre-convolution kernel case first (\(\hat{P}_{pre}=1\)) at all
frequencies) in \Cref{eq:K}. In this case, the matching kernel
converges to zero, the overall expression of \(\hat{K}\) converges to
\(F_{n}/\sigma_n\). This is the same type of solution as \(c_2\) in
\Cref{fig:theo_Gaussians_ft} and in \Cref{fig:theo_Gaussians_img_c2}.
When we convolve the template image, \(\sigma_r \ll \sigma_n\), we
don't expect any significant noise even if \(\hat{P}_{mk}\) is noisy
in its tails.
%
The science image convolution case is less obvious, \(\sigma_r \gg
\sigma_n\) can prevent \(\sigma_{n}\) from becoming the dominant term
in the denominator in \Cref{eq:K} and the numerical noise in the tails
of \(\hat{P}_{mk}\) may remain in the correction.
%
If we use a pre-convolution kernel however, the overall expression becomes
divergent in \Cref{eq:K}. Even if we don't directly hit numerical
overflows, K cannot be applied as a separate standalone step on an
already calculated difference image. In \Cref{eq:DaldPreMk}, we can
see that calculating the corrected difference image (\(\hat{D}_{AL+d}\))
is exactly the same problem as calculating the ZOGY difference image
in \Cref{eq:Dhat}. In these expressions both the numerators and the
denominators converge to zero which in practice result in high
frequency noise in the tail regions of the input PSFs, pre-convolution
and matching kernels. We face the very same numerical problems that was
discussed in \Cref{sec:ZOGYFFT}.
% 
\subsection{Decorrelation afterburner normalization}
\par To preserve the photometric scale of the AL difference image
(that is the same of the science image), the decorrelation afterburner
correction should be overall a convolution with a normalized
correction kernel (in image space). PSF normalization can be easily
ensured in frequency space. If we evaluate any PSF at 0 frequency,
...
decorrelation afterburner is written in \Cref{eq:KPre}.
%
\begin{align}
...
\label{eq:KPre}
\end{align}
\subsection{Proposed fusion of AL with ZOGY}
To keep the advantage of AL not assuming a prior knowledge of the
image PSFs and of ZOGY to produce a proper difference image, we
propose the following algorithmic fusion of the two approaches. Use AL
with an suitably chosen pre-convolution kernel to determine
\(P_{pre}\), \(P_{mk}\) in image space, perhaps in a spatially varying
manner for the whole image. This optimisation can focus solely on the
quality of the matching kernel solution. The AL difference image in
its original \Cref{eq:Dal} form needn't be calculated. In the next
step, using \Cref{eq:DaldPreMk}, we perform a ZOGY difference image
calculation using \(\hat{P}_{pre}\) and \(\hat{P}_{mk}\) like the
``image PSFs'' with all the precautions and numerical considerations
discussed earlier. If \(\hat{P}_{pre}\) is positive and real (as it is
in the case of a Gaussian PSF), it disappears completely from the
expression.
%
\section{Conclusions}
%
